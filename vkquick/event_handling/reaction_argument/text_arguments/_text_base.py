...
class TextBase:
    ...
    @staticmethod
    def check_valid_length(max_length: ty.Optional[int], min_length: int):
    ...
    def create_length_rule(self, desc):
    ...
    def check_range(
        ...
        if got_value is not vkquick.event_handling.reaction_argument.text_arguments.base.UnmatchedArgument:
            length = len(got_value)
            if length < self.min_length or (self.max_length is not None and length > self.max_length):
                return (
