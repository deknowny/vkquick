...
class API:
    ...
    API ВКонтакте представляет собой набор специальных "методов" для получения какой-либо информации. Например, чтобы отправить сообщение, нужно вызвать метод `messages.send` и передать соответствующие парамьеры (тектс, ID диалога и другие). Вызов метода и передача параметров представляет собой лишь отправку HTTP запроса. Вызыать метод `some.method` и передать туда параметр `foo=1` означает составить запрос такового вида: `https://api.vk.com/method/some.method?foo=1`. Составлять такие запросы каждый раз достаточно неудобно, поэтому этот класс предоставляет возможности по работе с API через более комфортный интерефейс 
    """
